...
usage_tib <- read_tsv("../05_DGE/5.1_starCAT/starCAT.scores.txt.gz") %>%
  rename("cell" = "...1")
tcat_celltypes_df <- data.frame(
  select(usage_tib, Multinomial_Label, Proliferation),
  row.names = usage_tib$cell
)
colnames(tcat_celltypes_df) <- c("TCAT_Multinomial_Label", "TCAT_Proliferation")
seu <- AddMetaData(seu, tcat_celltypes_df)
seu$TCAT_Multinomial_Label <- factor(
  seu$TCAT_Multinomial_Label,
  levels = c(
    "CD4_Naive",
    "CD4_CM",
    "CD4_EM",
...
# Add TCR calls
tcr_calls <- read_csv("../06_TCR_Diversity/7.1_TCR_calls.csv.gz")
...
seu$souporcell_origin <- factor(
  seu$souporcell_origin,
