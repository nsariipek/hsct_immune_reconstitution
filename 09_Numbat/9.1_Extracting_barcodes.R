...
for (patient in patients) {
  ...
  pre_transplant <- patient_df %>%
    filter(sample_status == "pre-transplant" & origin == "recipient")
  remission <- patient_df %>%
    filter(sample_status == "remission" & origin == "recipient")
  relapse <- patient_df %>%
    filter(sample_status == "relapse" & origin == "recipient")
  ...
  write_tsv(
    as.data.frame(barcode_pre),
    file = paste0(output_dir, patient, "_pre_recipient_barcodes.tsv"),
    col_names = FALSE
  )
  write_tsv(
    as.data.frame(barcode_rem),
    file = paste0(output_dir, patient, "_rem_recipient_barcodes.tsv"),
    col_names = FALSE
  )
  write_tsv(
    as.data.frame(barcode_rel),
    file = paste0(output_dir, patient, "_rel_recipient_barcodes.tsv"),
    col_names = FALSE
  )
