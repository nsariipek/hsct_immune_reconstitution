...
write.table(bmm_annotations, "../AuxiliaryFiles/250426_BMM_annotations.tsv", append = F, quote = F, sep = "\t", row.names = T, col.names = T)

# Add predicted cell type annotations into the Seurat object
seu = AddMetaData(seu, bmm_annotations[, c("mapping_error_QC", "predicted_CellType", "predicted_CellType_Broad", "predicted_Pseudotime")])
...
as_tibble(seu@meta.data) %>%
  group_by(patient_id, mapping_error_QC) %>% count() %>%
  group_by(patient_id) %>% mutate(proportion = n/sum(n)) %>%
...
add_granular_celltypes <- c("CD14 Mono", "CD16 Mono", "Immature B", "Mature B", "CD4 Naive", "CD4 Central Memory", "CD4 Effector Memory", "CD4 Regulatory", "CD8 Naive", "CD8 Central Memory", "CD8 Effector Memory 1", "CD8 Effector Memory 2", "CD8 Tissue Resident Memory", "T Proliferating", "NK CD56high", "NK Proliferating")
...
seu$predicted_CellType <- factor(seu$predicted_CellType,
  levels = intersect(celltype_colors_df$celltype, seu$predicted_CellType))
seu$predicted_CellType_Broad <- factor(seu$predicted_CellType_Broad,
...
DimPlot(seu_pass, group.by = "predicted_CellType", label = T,
  raster = T, raster.dpi = c(1536, 1536), pt.size = 3) +
  scale_color_manual(values = celltype_colors) +
  theme(aspect.ratio = 1)
ggsave("2.1.2_Granular_annotation.pdf", width = 16, height = 8)

DimPlot(seu_pass, group.by = "celltype", label = T,
  raster = T, raster.dpi = c(1536, 1536), pt.size = 3) +
  scale_color_manual(values = celltype_colors) +
  theme(aspect.ratio = 1)
ggsave("2.1.3_Final_annotation.pdf", width = 12, height = 8)

DimPlot(seu_pass, group.by = "celltype", label = F, split.by = "patient_id", ncol = 6) +
  scale_color_manual(values = celltype_colors) +
  theme(panel.border = element_rect(color = "black"),
        aspect.ratio = 1, title = element_blank())
