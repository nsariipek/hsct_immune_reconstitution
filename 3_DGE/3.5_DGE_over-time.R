...
ggsave("3.5.1_Cell_counts.pdf", width = 7, height = 4)
...
# Visualize
pdf("3.5.2_MA_plot_before_shrinking.pdf", width = 8, height = 6)
plotMA(res)
dev.off()
...
pdf("3.5.3_MA_plot_after_shrinking.pdf", width = 6, height = 6)
plotMA(res)
dev.off()
...
write_tsv(
  ...
  file = "3.5_DGE_Pre-transplant_vs_Relapse.txt"
)
...
# Save
ggsave("3.5.4_Volcano.pdf", volcano, width = 6, height = 4)
...
ggsave(
  paste0("3.5.5_Top10_relapse_boxplots.pdf"),
  plot = top_10_boxplot,
  width = 8,
  height = 6
)
...
# Plot heatmap
pdf("3.5.6_Top10_relapse_heatmap.pdf", width = 7, height = 6)
pheatmap::pheatmap(
  mat_scaled,
  cluster_rows = T,
  cluster_cols = T,
