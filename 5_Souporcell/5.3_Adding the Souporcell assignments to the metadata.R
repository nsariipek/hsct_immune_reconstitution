...
Df1 <- read_tsv(paste0(my_wd, "/TP53_ImmuneEscape/5_Souporcell/outputs/clusters/souporcell_Pt12_clusters.tsv"))
...
df12 <- P12 %>% left_join(Df1, by="barcode")
...
df12 <- subset(df12, subset = assignment %in% c("0","1")) 

# Rename the assignments, checking with pre-transplant samples which would be host cells
df12$assignment <- gsub("0","host", df11$assignment)
df12$assignment <- gsub("1", "donor", df11$assignment) 

#### Repeat this for each patient repeatedly
