...
meta_tib <- as_tibble(meta, rownames = "cell")
meta_tib %>% group_by(Sample) %>% mutate(total_n = n()) %>% group_by("Sample", "celltype") %>%
  dplyr::summarise(n = n(), total_n = total_n[1]) %>% # add sth like "neoa_score = mean(nea_score)" in the summarize function. Also, replace cell type by TCR column
  mutate(percent_n = n / total_n * 100) %>% view
...
p1 <- joined_tibble %>% filter(!duplicated(ptnumber)) %>%
  ...
  theme(aspect.ratio = 1.5, axis.text.x = element_text(angle = 45, vjust= 1, hjust = 1,    size = 15, color = "black"),
        ...
        stat_compare_means(aes(group = cohort), method = "t.test", method.args = list(var.equal = T),
                           label = "p.format", label.x = 1.5, label.y=20, tip.length = 1, size = 6)
...
p2 <- joined_tibble %>% filter(!duplicated(ptnumber)) %>%
  ...
  theme(aspect.ratio = 1.5, axis.text.x = element_text(angle = 45, vjust= 1, hjust = 1,    size = 15, color = "black"),
        ...
        legend.text = element_text(size = 12)) +
  stat_compare_means(aes(group = cohort), method = "t.test", method.args = list(var.equal = T),
                     label = "p.format", label.x = 2, label.y=200, tip.length = 1, size = 6)
