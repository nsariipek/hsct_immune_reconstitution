...
# For Nurefsan:
my_wd <- "/Users/dz855/Dropbox (Partners HealthCare)/ImmuneEscapeTP53/TP53_ImmuneEscape/6_TCR_Diversity"
# For Peter:
#my_wd <- "~/DropboxMGB/Projects/ImmuneEscapeTP53/TP53_ImmuneEscape/6_TCR_Diversity"

# Load the tsv files for relapse cohort
rel_files <- list.files(paste0(my_wd, "/../../1801 data/Relapse TCRs/"), full.names = T)
...
rem_files <- list.files(paste0(my_wd, "/../../1801 data/Remission TCRs/"), full.names = T)
...
rel_tb <- read_xlsx(paste0(my_wd, "/../../1801 data/Clinical tables/relapse cohort tcr data.xlsx"))
rem_tb <- read_xlsx(paste0(my_wd, "/../../1801 data/Clinical tables/remission cohort tcr data.xlsx"))
...
tac <-  subset(clinical_tb, treatment_arm == "Tac/MTX")
tac <- tac$Patient_ID
formatted_numbers <- ifelse(tac >= 10 & tac < 100, sprintf("%03d", tac), as.character(tac))
...
plot(x = rows_per_sample_subset, y = rows_per_sample2, log = "xy", xlab = "Original row number", ylab = "After expanding")
...
m <- compute_diversity(expanded_list, cloneCall = "rearrangement", n.boots = 1000)
View(m)
...
extracted_columns <- lapply(expanded_list, function(df) df[1,"sample_simpson_clonality"])
t_df <- data.frame("Patient_ID" = names(extracted_columns), "simpson_clonality" = unlist(extracted_columns), row.names = NULL)
...
t1_filtered <- t1 %>%
  filter(!(cohort == "remission" & Overall.Survival.Months.Post.Transplant < 11 & Overall.Survival.Post.Transplant.Event == "Death"))
...
p1 <- ggplot(data = t1_filtered_final,aes(x = inverse_simpson_index, y = inv.simpson))+
      geom_point(size=4)+
      theme_bw()+
      ylab("Inverse Simpson Index/calculated")+
      xlab("Inverse Simpson Index/converted")+
      theme(aspect.ratio = 0.5, axis.text.x = element_text(angle = 45, vjust= 1, hjust = 1, size =        15, color = "black"),
        axis.title.x = element_text(size = 15, color = "black"), 
        axis.text.y = element_text(size = 15),
        axis.title.y = element_text(size = 15, color = "black"),
        legend.key.size = unit(3,"mm"),
...
p2 <- t1_filtered_final %>%
  filter(!duplicated(Patient_ID)) %>%
    ...
    theme(aspect.ratio = 1.5, axis.text.x = element_text(angle = 45, vjust= 1, hjust = 1, size = 15, color = "black"),
        ...
        legend.title = element_text(size = 14),
        legend.text = element_text(size = 12)) + 
  stat_compare_means(aes(group = cohort), method = "wilcox", method.args = list(var.equal = T), label = "p.format",
                     label.x = 1.5, label.y = 300, tip.length = 1, size = 4)
p2
...
p3 <- t1_filtered_final %>% 
  ...
  theme(aspect.ratio = 1.5, axis.text.x = element_text(angle = 45, vjust= 1, hjust = 1, size = 15, color = "black"),
        ...
        legend.text = element_text(size = 12))
+ 
  stat_compare_means(aes(group = cohort), method = "wilcox", method.args = list(var.equal = T),label = "p.format", label.x = 1.5, label.y=300, tip.length = 1, size = 4)
p3
